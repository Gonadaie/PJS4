 ...
 */
  function update_match($id1, $id2){
    $db = db_connect();
    if($db) {
      $query_update_match_first = "UPDATE match SET liked_by_god_son = 1, result = true, liked_by_god_father = 1 WHERE id_student_god_father = :id1 AND id_student_god_son = :id2";
  ...
  function insert_match_first($id1, $id2){
    $db = db_connect();
    if($db) {
      $query_set_match_first = "INSERT INTO match(id_student_god_son, id_student_god_father, liked_by_god_son) VALUES(:id1,:id2, 1)";
  ...
  function insert_match_second($id1, $id2){
    $db = db_connect();
    if($db) {
      $query_set_match_first = "INSERT INTO match(id_student_god_son, id_student_god_father, liked_by_god_father) VALUES(:id1,:id2, 1)";
      $statement_set_match_first = $db->prepare($query_set_match_first);
      $statement_set_match_first->bindValue(':id1', $id1);
      $statement_set_match_first->bindValue(':id2', $id2);
      $statement_set_match_first->execute();
