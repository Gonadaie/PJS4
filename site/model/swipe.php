...
function reset_dislike($student){
	...
	if($student->getYear() == 1){
		$query = "UPDATE student_match SET liked_by_god_son = -1 WHERE result = false AND student_id_god_son = :id";
		$statement = $db->prepare($query);
		$statement->bindValue(':id', $student->getId());
		$statement->execute();
	}
	else{
		$query = "UPDATE student_match SET liked_by_god_father = -1 WHERE result = false AND student_id_god_father = :id";
		$statement = $db->prepare($query);
		$statement->bindValue(':id', $student->getId());
		$statement->execute();
	}
