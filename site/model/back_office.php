...
function get_all_student(){
  ...
  $list = array();
  if($db){
    $query = "SELECT email, year FROM student where admin = false and validate_account = true ORDER BY year, email";
...
function get_couples_without_name(){
  $db = db_connect();
  if($db) {
    ...
    $couples = array();
    while($row = $statement->fetch(PDO::FETCH_ASSOC)){
      array_push($couples, array(decrypt_data($row['email1']),
      decrypt_data($row['email2'])));
    }
  }
  return $couples;
}
...
function get_couples(){
  ...
  if($db) {
    ...
    while($row = $statement->fetch(PDO::FETCH_ASSOC)){
      ...
      decrypt_data($row['email2']),decrypt_data($row['name2'])));
    }
  }
  return $couples;
}
